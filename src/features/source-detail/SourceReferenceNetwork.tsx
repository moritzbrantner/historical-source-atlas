import { Surface, SurfaceContent, SurfaceHeader, SurfaceTitle } from "@moritzbrantner/ui";
...
export function SourceReferenceNetwork({ source }: { source: HistoricalSource }) {
  return (
    <Surface>
      <SurfaceHeader>
...
function ReferenceColumn({
  ...
  const toneClassName =
    tone === "incoming" ? "bg-blue-50 text-blue-700" : "bg-teal-50 text-teal-700";
    ...
    <section className="grid gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
      ...
      <ul className="grid gap-2">
        {items.map((item) => (
          ...
          >
            <span className={`w-fit rounded-full px-2 py-1 text-xs font-bold ${toneClassName}`}>
              {item.relation}
            </span>
