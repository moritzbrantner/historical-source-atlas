...
test("unknown source route offers source suggestions", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Dead Sea Scrolls" })).toBeVisible();
...
test("source page back button and browser history return between views", async ({ page }) => {
  await page.goto("/");
  await page
...
test("source pages have no serious or critical accessibility violations", async ({ page }) => {
  await page.goto("/sources/dead-sea-scrolls");
