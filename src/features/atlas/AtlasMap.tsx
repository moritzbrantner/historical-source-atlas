...
export function AtlasMap({
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (event: PointerEvent) => {
      ...
      const source = getNearestSourceAtPoint(panel, sources, event.clientX, event.clientY);
...
export function SourcePopup({ feature }: { feature: HistoricalSourceFeature }) {
  ...
  return (
    <div className="grid min-w-52 gap-1">
      ...
      <span className="text-sm text-slate-600">Found: {properties.discovered}</span>
    </div>
  );
}
...
function SourceReferenceTooltip({
  ...
  return (
    <div className="grid max-w-64 gap-1">
      ...
      <span className="text-sm text-slate-600">{feature.flow.properties.relation}</span>
    </div>
  );
}
...
function SourceReferencePopup({
  ...
  return (
    <div className="grid max-w-64 gap-1">
      ...
      <span className="text-sm text-slate-600">{feature.flow.properties.relation}</span>
      <span className="text-sm text-slate-600">{feature.flow.properties.note}</span>
    </div>
  );
}
