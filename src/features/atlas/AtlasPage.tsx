...
export function AtlasPage({ onOpenSource }: { onOpenSource: (sourceId: string) => void }) {
    ...
    return (
      <PageShell maxWidth="wide">
        <ErrorState className="rounded-lg border border-slate-200 bg-white">
          ...
          <StateViewDescription>Refresh the page and try again.</StateViewDescription>
        </ErrorState>
      </PageShell>
    );
  }
  ...
  return (
    <PageShell className="min-h-screen" maxWidth="full">
      <PageHeader className="gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="mb-2 text-xs font-bold uppercase text-teal-700">Historical source atlas</p>
          ...
          <PageDescription className="sr-only">
            Explore historical texts, artifacts, inscriptions, and manuscripts by discovery location
            and date.
          </PageDescription>
        </div>
        <PageActions>
