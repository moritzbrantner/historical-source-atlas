...
test("atlas loads with source list, map, and default source", async ({ page }) => {
  await page.goto("/");
...
test("source kind filters update the visible source list", async ({ page }) => {
  ...
  const sourceTypeScope = page.locator("fieldset").filter({ hasText: "Source type scope" });
  ...
  await expect(page.getByRole("button", { name: "Clear sourceKinds filter" })).toBeVisible();
...
test("reference direction filters update map link legend", async ({ page }) => {
  ...
  await expect(page.getByLabel("Map legend").getByText("References")).toBeVisible();
  await expect(page.getByLabel("Map legend").getByText("Referenced by")).toBeVisible();

  await mapLinks.getByRole("checkbox", { name: "All" }).click();

  await expect(page.getByLabel("Map legend").getByText("References")).toHaveCount(0);
  await expect(page.getByLabel("Map legend").getByText("Referenced by")).toHaveCount(0);
...
test("source list selection updates details and source-page navigation", async ({ page }) => {
  await page.goto("/");

  const sourceList = page.getByLabel("Source list");
  const rosettaStone = sourceList.getByRole("button", { name: /Rosetta Stone/ });
...
test("atlas has no serious or critical accessibility violations", async ({ page }) => {
  await page.goto("/");

  await expectNoA11yViolations(page);

  await expect(page.locator("fieldset").filter({ hasText: "Source type scope" })).toBeVisible();
  await expectNoA11yViolations(page);
});
