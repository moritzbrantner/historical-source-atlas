...
export function AtlasSidebar({
  ...
  return (
    <aside className="grid gap-4 lg:sticky lg:top-4" aria-label="Source details">
      <MetricStats
        stats={[
      ...
      <SourceDetail source={selectedSource} onOpenPage={onOpenSource} />
          ...
          <div className="grid max-h-[330px] gap-2 overflow-auto pr-1">
            {sources.map((source) => (
              <Button
                ...
                key={source.id}
                type="button"
                variant={source.id === selectedSourceId ? "default" : "secondary"}
                {...(source.id === selectedSourceId ? { className: selectedSourceClassName } : {})}
                onClick={() => {
                  onSelectSource(source.id);
                }}
              >
                <span className="min-w-0 flex-1 truncate text-left">{source.label}</span>
...
function Fact({ label, value }: { label: string; value: string }) {
  return (
    <div className="grid gap-1 border-t border-slate-200 pt-3">
      ...
      <dd className="m-0 text-sm font-semibold leading-5 text-slate-800">{value}</dd>
    </div>
  );
}
