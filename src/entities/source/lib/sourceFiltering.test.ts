...
const sources: HistoricalSource[] = [
  sourceFixture({
    ...
    kind: "manuscript",
    label: "Alpha Tablet",
    references: [{ label: "Beta Chronicle", note: "Points to beta", relation: "quotes" }],
  }),
  sourceFixture({
    discoveredYear: 1905,
    id: "beta-chronicle",
    ...
    references: [{ label: "Gamma Inscription", note: "Second hop", relation: "cites" }],
    region: "Delta Region",
    sourceYear: -200,
  ...
  sourceFixture({
    ...
    references: [{ label: "Alpha Tablet", note: "Incoming to alpha", relation: "comments on" }],
    referencedIn: [
      { label: "Zeta Catalogue", note: "Referenced-in searchable", relation: "lists" },
    ],
    sourceYear: -100,
  }),
  sourceFixture({
...
describe("sourceFiltering", () => {
  it("returns all sources with default filters and full timeline ranges", () => {
    expect(filterSources(sources, createFilters()).map((source) => source.id)).toEqual(
      sources.map((source) => source.id),
    );
  });

  it("matches query text across source and relationship fields case-insensitively", () => {
    expect(filterSources(sources, createFilters({ query: " alpha " })).map(ids)).toEqual([
      "alpha-tablet",
      "delta-commentary",
    ]);
    expect(filterSources(sources, createFilters({ query: "beta archive" })).map(ids)).toEqual([
      "beta-chronicle",
    ]);
    expect(filterSources(sources, createFilters({ query: "delta region" })).map(ids)).toEqual([
      "beta-chronicle",
    ]);
    expect(filterSources(sources, createFilters({ query: "SEARCH-ONLY" })).map(ids)).toEqual([
      "beta-chronicle",
    ]);
    expect(
      filterSources(sources, createFilters({ query: "referenced-in searchable" })).map(ids),
  ...
  it("sorts by timeline year and then label", () => {
    expect(sortSourcesByTimeline([sources[4]!, sources[3]!], "source").map(ids)).toEqual([
      "delta-commentary",
      "epsilon-archive",
    ]);
  });
