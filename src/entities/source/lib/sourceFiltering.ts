...
export function filterSources(sources: HistoricalSource[], filters: SourceFilters) {
  ...
  return sources.filter((source) => {
    ...
    const matchesTimeline = (["discovery", "source"] as const).every((mode) =>
      isYearInRange(getTimelineYear(source, mode), filters.timelineRanges[mode]),
    );
    const matchesQuery =
...
function createSourceRelationGraph(sources: HistoricalSource[]): SourceRelationGraph {
...
function sourceReferencesCandidate(source: HistoricalSource, candidate: HistoricalSource) {
...
function relationshipTargetsSource(relationshipLabel: string, source: HistoricalSource) {
  ...
  for (let currentDepth = 0; currentDepth < Math.max(1, depth); currentDepth += 1) {
    ...
    frontier.forEach((sourceId) => {
      adjacency.get(sourceId)?.forEach((relatedSourceId) => {
        if (visited.has(relatedSourceId) || relatedSourceId === selectedSourceId) {
          return;
        }
