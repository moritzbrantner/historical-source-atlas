...
describe("staticSourceRepository", () => {
  it("lists static atlas sources", async () => {
    await expect(staticSourceRepository.listAtlasSources()).resolves.toBe(historicalSources);
  });

  it("returns a source by slug", async () => {
    await expect(staticSourceRepository.getSourceBySlug("dead-sea-scrolls")).resolves.toMatchObject(
      {
        id: "dead-sea-scrolls",
  ...
  it("returns null for unknown slugs", async () => {
    await expect(staticSourceRepository.getSourceBySlug("unknown-source")).resolves.toBeNull();
  });
