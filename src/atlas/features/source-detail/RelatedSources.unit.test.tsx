...
describe('RelatedSources', () => {
  it('shows source context facts', () => {
    ...
    expect(
      screen.getByText(`${currentSource.properties.discoveredYear}`),
    ).toBeInTheDocument();
  ...
  it('shows at most three related sources from the same region and excludes the current source', () => {
    ...
    expect(
      screen.queryByRole('button', { name: /Current Source/ }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: /Related One/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: /Related Two/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: /Related Three/ }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole('button', { name: /Related Four/ }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole('button', { name: /Other Region Source/ }),
    ).not.toBeInTheDocument();
  });
