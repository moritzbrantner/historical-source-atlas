...
export function AtlasFilters({
  ...
  onReferenceDirectionFiltersChange: (
    filters: SourceReferenceDirection[],
  ) => void;
  ...
  >(() => new Set());
  const [collapsedRelationshipFilters, setCollapsedRelationshipFilters] =
...
function SourceKindScopeControl({
  ...
  onReferenceFilterCollapsedChange: (
    referenceKind: RelationshipFilterKey,
  ) => void;
}) {
  const allId = useId();
  const bodyId = useId();
            ...
            <Checkbox
              ...
              onCheckedChange={(checked) => {
                onFilterChange({
                  ...
                  referenced:
                    checked === true
                      ? { ...filter.referenced, enabled: false }
                      : filter.referenced,
                  referencing:
                    checked === true
...
function getSourceKindFilterLabel(filters: SourceKindFilters) {
  const scopedKinds = allSourceKinds.filter((kind) =>
    isCustomizedSourceKindFilter(filters[kind]),
  );
...
function getReferenceDirectionFilterLabel(filters: SourceReferenceDirection[]) {
  ...
  return (
    referenceDirectionOptions.find((option) => option.direction === filters[0])
      ?.label ?? 'Links'
  );
}
...
function getReferenceFilterCollapseKey(
  kind: SourceKind,
  referenceKind: RelationshipFilterKey,
) {
