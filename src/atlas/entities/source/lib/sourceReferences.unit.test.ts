...
describe('sourceReferences', () => {
  it('creates incoming and outgoing flows for known labels', () => {
    ...
    expect(flows.some((flow) => flow.properties.direction === 'incoming')).toBe(
      true,
    );
    expect(flows.some((flow) => flow.properties.direction === 'outgoing')).toBe(
      true,
    );
  });
  ...
  it('sets incoming direction from reference location to source location', () => {
    ...
    expect(incoming.to).toEqual([
      deadSeaScrolls.longitude,
      deadSeaScrolls.latitude,
    ]);
  });
